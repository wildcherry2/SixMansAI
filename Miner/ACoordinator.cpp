...
void Components::ACoordinator::SerializeMessagesInFile(const path& file) const {
    ...
    Log("Serializing from {}...", file.filename().string());
    try {
        ifstream js_file(file);
        json parent;
        js_file >> parent;
        for(auto& msg : parent["messages"]) {
            if(auto current = dynamic_pointer_cast<AMessage>(message_serializer->Serialize(msg)))
                core->raw_messages.push_back(current);
        }

        Log("Messages from {} serialized!", file.filename().string());
        ...
        for(auto it = core->raw_messages.begin(); it != core->raw_messages.end(); ++it) {
            if(const auto next =(it + 1); next != core->raw_messages.end() && it->get()->IsBot() && (it + 1)->get()->IsBot()) {
                ...
                if(const auto next_type = next->get()->GetType(); this_type == next_type)
                    err++;
            }
        }

        Log("Error queues in {}: {} / {}", file.filename().string(), err, core->raw_messages.size());
    }
