...
namespace Database.Database.DatabaseCore.Factories {
    public class PlayerFactory : ILogger {
            ...
            for (var i = 2; i < chat_messages.messages.Count - 2; i++) {
                ...
                if (FollowsValidPattern(ref previous_previous, ref previous, ref current, ref next, ref next_next)) {
                    ...
                    if (ReferenceEquals(search, null)) {
                        ...
                        if (!search.HasName(current.author.name)) { search.TryAddName(current.author.name); }

                        if (!search.HasName(current.author.nickname)) { search.TryAddName(current.author.nickname); }

                        if (link != null && !search.HasName(link)) { search.TryAddName(link); }
                    }
                }
            }
        ...
        private bool FollowsValidPattern(ref DDiscordMessage previous_previous, ref DDiscordMessage previous, ref DDiscordMessage current, ref DDiscordMessage next, ref DDiscordMessage next_next) {
            if (current.type == EMessageType.PLAYER_Q &&
                (previous.IsBotNotification() || previous.IsBotResponse()) &&
                next.IsBotResponse() &&
                !previous_previous.IsBotNotification() &&
                !previous_previous.IsBotResponse() &&
                !next_next.IsBotResponse()) { return true; }
            return false;
        }
    }
}