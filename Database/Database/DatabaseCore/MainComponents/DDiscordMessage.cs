...
namespace Database.Database.DatabaseCore.MainComponents {
    public class DDiscordMessage : IDatabaseComponent {
        ...
        [JsonPropertyName("author")] public FAuthor? author { get; init; }

        [JsonPropertyName("reactions")] public List<FReaction>? reactions { get; init; }

        [JsonPropertyName("mentions")] public List<FAuthor>? mentions { get; init; }

        [JsonPropertyName("embeds")] public List<FEmbed>? embeds { get; init; }

        [JsonIgnore] public EMessageType type { get; set; } = EMessageType.UNKNOWN;
        ...
        public string? GetEmbeddedTitle() { return embeds != null && embeds.Count > 0 && embeds[0].title != null ? embeds[0].title : null; }

        public string? GetEmbeddedDescription() { return embeds != null && embeds.Count > 0 ? embeds[0].description : null; }

        public FField? GetEmbeddedField(in int index) { return embeds != null && embeds.Count > 0 && embeds[0].fields.Count >= index + 1 ? embeds[0].fields[index] : null; }
        ...
        public string? GetPlayerNameFromEmbeddedLink(in string? embedded_link) {
            if (embedded_link != null) {
                ...
                if (pre_match.Success) { return pre_match.Value; }
            }
        ...
        public bool HasSubstitutes() { return IsScoreReportMessage() && mentions != null && mentions.Count == 2; }
        ...
        public string[]? GetPlayerNamesFromTeamPickedMessage() {
            ...
            if (str_name != null) { ret[1] = str_name; }
            else { return null; }

            str_name = GetPlayerNameFromEmbeddedLink(link_strings[2].Trim());
            if (str_name != null) { ret[2] = str_name; }
            else { return null; }
            ...
            if (str_name != null) { ret[3] = str_name; }
            else { return null; }

            str_name = GetPlayerNameFromEmbeddedLink(link_strings[1].Trim());
            if (str_name != null) { ret[4] = str_name; }
            else { return null; }

            str_name = GetPlayerNameFromEmbeddedLink(link_strings[2].Trim());
            if (str_name != null) { ret[5] = str_name; }
            else { return null; }
        ...
        private bool IsQMessage() { return content != null && content == @"!q"; }

        private bool IsLeaveMessage() { return content != null && content == @"!leave"; }
        ...
        private bool IsScoreReportMessage() { return IsAuthorHuman() && content != null && RegularExpressions.score_report_relaxed_regex.IsMatch(content); }

        private bool IsTeamsPickedMessage() { return IsAuthorBot() && GetEmbeddedDescription() == "You may now join the team channels"; }

        // With PlayerFactory, if this is true then reset the counter, since captains not picking resets the queue
        private bool IsLobbyCancelledNoPickMessage() { return IsAuthorBot() && content != null && content.Contains("Captain failed to pick players for **Match ID"); }

        private bool IsAuthorBot() { return author != null && author.isBot != null && author.isBot.Value; }

        private bool IsAuthorHuman() { return !IsAuthorBot(); }

        // Returns true if this message is a bot response to a player !q or !leave command
        public bool IsBotResponse() { return IsBotResponseToPlayerQ() || IsBotResponseToPlayerLeave(); }

        // Returns true if this message is a voting complete or teams picked message
        public bool IsBotNotification() { return IsVotingCompleteMessage() || IsTeamsPickedMessage() || IsLobbyCancelledNoPickMessage(); }

        private bool IsBotResponseToPlayerQ() { return IsAuthorBot() && embeds != null && embeds.Count > 0 && embeds[0].description.Contains(") has joined."); }

        private bool IsBotResponseToPlayerLeave() { return IsAuthorBot() && embeds != null && embeds.Count > 0 && embeds[0].description.Contains(") has left (using command)."); }
        ...
        protected override bool IsEqual(IDatabaseComponent? rhs) {
            ...
            if (rhs_casted) { return id == rhs_casted!.id; }

            return false;
        }
        protected override bool IsLessThan(IDatabaseComponent? rhs) {
            var rhs_casted = rhs != null ? rhs as DDiscordMessage : null;
            if (rhs_casted) { return ulong.Parse(id!) < ulong.Parse(rhs_casted!.id!); }
