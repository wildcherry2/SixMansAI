...
public class ScoreReport : IDatabaseComponent {
    public ScoreReport() { }
    public ScoreReport(in PrimaryKey key) {
        primary_key      = key;
    ...
    private bool bIsPrimaryKeySet { get; set; }

    public bool bError { get; set; } = false;
    public bool IsValid() {
        if (iMatchId == -1) {
            logger.Log("Match ID invalid in score report!");

            return false;
        }

        if (ReferenceEquals(reporter, null)) {
            logger.Log("Null reporter in score report for lobby {0}!", iMatchId.ToString());

            return false;
        }

        if (bHasSubs) {
            if (ReferenceEquals(subbed_in, null)) {
                logger.Log("Score report for lobby {0} has subs but subbed_in player was null!", iMatchId.ToString());

                return false;
            }

            if (ReferenceEquals(subbed_out, null)) {
                logger.Log("Score report for lobby {0} has subs but subbed_out player was null!", iMatchId.ToString());
