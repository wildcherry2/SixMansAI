...
public class ChatCleaner : CleanerBase {
    private ChatCleaner(in bool bUsingDirectory = false) { this.bUsingDirectory = bUsingDirectory; }
    ...
    public void ProcessChat(in string override_path = "") {
        ...
        else { logger.Log("Preconditions not met! Score report chat data was not set!"); }

        if (filtered_messages.messages.Count > 0)
            bIsComplete = true;
        else
            logger.Log("Error cleaning chat data! No messages were filtered!");
