...
namespace Database.Database.Structs {
    public class FScoreReport : ILogger {
        ...
        public bool IsValid() {
            if (iMatchId == -1) {
                Log("Match ID invalid in score report!");

                return false;
            }

            if (ReferenceEquals(reporter, null)) {
                Log("Null reporter in score report for lobby {0}!", iMatchId.ToString());
            ...
            if (bHasSubs) {
                if (ReferenceEquals(subbed_in, null)) {
                    Log("Score report for lobby {0} has subs but subbed_in player was null!", iMatchId.ToString());

                    return false;
                }

                if (ReferenceEquals(subbed_out, null)) {
                    Log("Score report for lobby {0} has subs but subbed_out player was null!", iMatchId.ToString());
