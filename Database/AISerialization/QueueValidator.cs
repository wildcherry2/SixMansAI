...
public static class QueueValidator {
    public static bool IsQueueValid(in DQueue queue) {

        return IsWinnerSet(queue) && HasDate(queue) && ArePlayersSet(queue) && ArePlayersValid(queue);
    }
    ...
    private static bool IsWinnerSet(in DQueue queue) {
        return queue.winner != ETeamLabel.NOT_SET;
    }

    private static bool HasDate(in DQueue queue) {
        return queue.teams_picked_message.timestamp != null;
    }
}