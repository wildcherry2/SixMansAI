...
public class QueueReportBinder : ILogger {
    private QueueReportBinder() : base(ConsoleColor.Yellow, 1, "QueueReportBinder") { problems = new List<DQueue>(); }
    ...
    private void SetWinnerInQueue(in DQueue queue) {
        ...
        if (team_one_not_null && !ReferenceEquals(queue.team_one.player_one, null) && queue.team_one.player_one.discord_id == report.reporter.discord_id) { queue.winner          = ETeamLabel.TEAM_ONE; }
        else if (team_one_not_null && !ReferenceEquals(queue.team_one.player_two, null) && queue.team_one.player_two.discord_id == report.reporter.discord_id) { queue.winner     = ETeamLabel.TEAM_ONE; }
        else if (team_one_not_null && !ReferenceEquals(queue.team_one.player_three, null) && queue.team_one.player_three.discord_id == report.reporter.discord_id) { queue.winner = ETeamLabel.TEAM_ONE; }
        else if (team_two_not_null && !ReferenceEquals(queue.team_two.player_one, null) && queue.team_two.player_one.discord_id == report.reporter.discord_id) { queue.winner     = ETeamLabel.TEAM_TWO; }
        else if (team_two_not_null && !ReferenceEquals(queue.team_two.player_two, null) && queue.team_two.player_two.discord_id == report.reporter.discord_id) { queue.winner     = ETeamLabel.TEAM_TWO; }
